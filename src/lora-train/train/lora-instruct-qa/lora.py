...
logging.basicConfig(filename='training-run.txt', level=logging.DEBUG)
...
class Dataset:
    ...
    def __init__(self, path: Path, key: str = "input"):
        if not path.exists():
            self._data = None
        else:
...
def load(args):
    dataset = load_from_disk("../../collection-scripts/instruct/data/processed_data/mali_llama3_instruct_dataset")
...
def iterate_batches(dset, tokenizer, batch_size, train=False):
    ...
    while True:
        ...
        for i in range(0, len(indices) - batch_size + 1, batch_size):
            # Encode batch
            batch = [tokenizer.encode(dset[int(indices[i + j])]['input'][0]) for j in range(batch_size)]
