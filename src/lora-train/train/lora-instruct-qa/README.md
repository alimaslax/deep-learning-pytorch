...
For example, the following will make a 4-bit quantized Llama 78 and by default
store it in `mlx_model`:

```
python convert.py --hf-path meta-llama/Meta-Llama-3-8B-Instruct -q
```
...
```
python lora.py --model mlx-community/Meta-Llama-3-8B-Instruct-4bit \
               --train \
               --iters 600
```
